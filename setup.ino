...
#include <Wire.h>    // I2C (gyro)
#include <EEPROM.h>  // persistent storage for calibration values
...
void setup() {
  ...
  // Enable pin-change interrupts for receiver inputs (digital 8..11 -> PCINT0..3)
  PCICR |= (1 << PCIE0);
  PCMSK0 |= (1 << PCINT0);
  PCMSK0 |= (1 << PCINT1);
  PCMSK0 |= (1 << PCINT2);
  ...
  Serial.begin(57600);     // serial for setup feedback
  delay(250);              // allow gyro to power on
}
...
void loop() {
  ...
  if (error == 0) {
    ...
    wait_for_receiver(); // wait up to 10s for valid receiver pulses
    Serial.println(F(""));
  }
  ...
  if (error == 0) {
    ...
    register_min_max(); // capture min/max endpoints
    Serial.println(F(""));
    Serial.println(F(""));
  ...
  if (error == 0) {
    ...
    if (type == 0) {
      ...
      Serial.println(F("==================================================="));
      start_gyro(); // configure gyro registers
    }
  }
  ...
  if (error == 0) {
    ...
    Serial.println(F("==================================================="));
    Serial.println(F("Do not move the quadcopter!! Calibration starts in 3 seconds"));
    delay(3000);
    Serial.println(F("Calibrating gyro, this may take ~8 seconds"));
    Serial.print(F("Please wait"));
    // gather samples to compute gyro offsets
    ...
    Serial.println(F(""));
    Serial.print(F("Axis 1 offset=")); Serial.println(gyro_roll_cal);
    Serial.print(F("Axis 2 offset=")); Serial.println(gyro_pitch_cal);
    Serial.print(F("Axis 3 offset=")); Serial.println(gyro_yaw_cal);
    Serial.println(F(""));
  ...
  if (error == 0) {
    ...
    Serial.println(F("See www.brokking.net Q & A for more on errors."));
  }
...
// ---------------------------
// I2C gyro probe: read a register (WhoAmI or similar)
// ---------------------------
...
// ---------------------------
// Configure gyro registers based on detected type
// ---------------------------
void start_gyro() {
  if (type == 2 || type == 3) { // L3G4200D or L3GD20H
...
// ---------------------------
// Read gyro angular rates and apply calibration offset if available
// ---------------------------
void gyro_signalen() {
  if (type == 2 || type == 3) {
    Wire.beginTransmission(address);
...
// ---------------------------
// Detect which receiver channel moved (30s timeout)
// ---------------------------
...
// ---------------------------
// Wait for the commanded stick movement to continue
// ---------------------------
...
// ---------------------------
// Wait until all sticks are near center
// ---------------------------
...
// ---------------------------
// Wait until receiver gives valid pulses (10s timeout)
// ---------------------------
...
// ---------------------------
// Measure extremes while sticks are moved
// ---------------------------
...
// ---------------------------
// Check for physical gyro motion (10s timeout)
// ---------------------------
void check_gyro_axes(byte movement) {
  ...
  while (timer > millis() && gyro_angle_roll > -30 && gyro_angle_roll < 30 && gyro_angle_pitch > -30 && gyro_angle_pitch < 30 && gyro_angle_yaw > -30 && gyro_angle_yaw < 30) {
    ...
    delayMicroseconds(3700); // ~250Hz loop timing
  }
  ...
  if ((gyro_angle_roll < -30 || gyro_angle_roll > 30) && gyro_angle_pitch > -30 && gyro_angle_pitch < 30 && gyro_angle_yaw > -30 && gyro_angle_yaw < 30) {
    ...
    trigger_axis = (gyro_angle_roll < 0) ? 0b10000001 : 0b00000001;
  }
  ...
  if ((gyro_angle_pitch < -30 || gyro_angle_pitch > 30) && gyro_angle_roll > -30 && gyro_angle_roll < 30 && gyro_angle_yaw > -30 && gyro_angle_yaw < 30) {
    ...
    trigger_axis = (gyro_angle_pitch < 0) ? 0b10000010 : 0b00000010;
  }
  ...
  if ((gyro_angle_yaw < -30 || gyro_angle_yaw > 30) && gyro_angle_roll > -30 && gyro_angle_roll < 30 && gyro_angle_pitch > -30 && gyro_angle_pitch < 30) {
    ...
    trigger_axis = (gyro_angle_yaw < 0) ? 0b10000011 : 0b00000011;
  }
...
// ---------------------------
// Pin-change interrupt for receiver PWM inputs (digital 8..11)
// ---------------------------
ISR(PCINT0_vect) {
  current_time = micros();
